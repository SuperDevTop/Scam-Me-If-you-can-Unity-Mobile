...
public class Board : MonoBehaviour {
	...
	void DestroyMatchesAt(int column, int row) {
		if(allDots[column, row].GetComponent<Dot>().isMatched) {
			...
			if(KeepData.currentScene == "Tutorial") {
			...
			//if(KeepData.currentScene == "Level 3") {
			//	if(allDots[column, row].tag == "Blue Dot" || allDots[column, row].tag == "Yellow Dot" || allDots[column, row].tag == "Pink Dot") {
			//		thePowerupBar.IncreaseScore(powerUpAmt);
			//	}
			//}

			//if(KeepData.currentScene == "Level 4") {
			//	if(allDots[column, row].tag == "Red Dot" || allDots[column, row].tag == "Green Dot" || allDots[column, row].tag == "Blue Dot" || allDots[column, row].tag == "Yellow Dot" || allDots[column, row].tag == "Pink Dot") {
			//		thePowerupBar.IncreaseScore(powerUpAmt);
			//	}
			//}
			...
			//GameObject score = Instantiate(scoreDisplay, allDots[column, row].transform.position, Quaternion.identity);
			//Destroy(score, 1f);
	...
	public void Goals() {
		...
		//if(scoreManager.score >= goals[1]) {
		//	goalReached = true;
		//}

		//if(scoreManager.score >= goals[2]) {
		//	goalReached = true;
		//}
	}
	...
	//public void SendSerialSignal() {
	//	toggle = !toggle; // Toggles on and off at each button click

	//	if(toggle) {
	//		//buttonText.text = "Servo Turn Clockwise";
	//		Debug.Log("Sending 1...");
